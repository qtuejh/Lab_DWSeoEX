...
cps <- VCorpus(VectorSource(data_oc_pre$article))
...
cps = tm_map(cps, myRemove, '위원장')
cps = tm_map(cps, myRemove, '신동빔')
cps = tm_map(cps, myRemove, '마지막')
cps = tm_map(cps, myRemove, '최고경영자')
cps = tm_map(cps, myRemove, '의사결정')
...
lda_data_term %>% arrange(topic,desc(beta)) %>% 
  openxlsx::write.xlsx("../result/oc/pre_covid/붙임1.lda_beta_precovid.xlsx")
...
tidy(lda_data,matrix = "gamma") %>% 
  ...
  openxlsx::write.xlsx("../result/oc/pre_covid/붙임2.토픽별대표문서_precovid.xlsx")

tidy(lda_data,matrix = "gamma") %>% 
  group_by(topic) %>% 
  slice(which.max(gamma)) %>% 
  openxlsx::write.xlsx("../result/oc/pre_covid/붙임3.토픽별대표문서리스트_precovid.xlsx")
...
tidy(lda_data,matrix = "gamma") %>% 
  ...
  openxlsx::write.xlsx("../result/oc/pre_covid/붙임4.문서별토픽_precovid.xlsx")
...
tidy(lda_data,matrix = "gamma") %>% 
  ...
  arrange(topic) %>% 
  openxlsx::write.xlsx("../result/oc/pre_covid/붙임5.토픽출현율_평균_precovid.xlsx")
...
data_oc_covid = data_oc %>% mutate(
  article = str_replace_all(keyword,","," ")
) %>% filter(covid_type=="covid19")


cps <- VCorpus(VectorSource(data_oc_covid$article))
cps = tm_map(cps, removePunctuation)
cps = tm_map(cps, removeNumbers)
...
lda_data_term %>% arrange(topic,desc(beta)) %>% 
  openxlsx::write.xlsx("../result/oc/covid19/붙임1.lda_beta_covid19.xlsx")
...
tidy(lda_data,matrix = "gamma") %>% 
  ...
  openxlsx::write.xlsx("../result/oc/covid19/붙임2.토픽별대표문서_covid19.xlsx")

tidy(lda_data,matrix = "gamma") %>% 
  group_by(topic) %>% 
  slice(which.max(gamma)) %>% 
  openxlsx::write.xlsx("../result/oc/covid19/붙임3.토픽별대표문서리스트_covid19.xlsx")
...
tidy(lda_data,matrix = "gamma") %>% 
  ...
  slice(which.max(gamma)) %>% 
  arrange(as.numeric(document)) %>% 
  openxlsx::write.xlsx("../result/oc/covid19/붙임4.문서별토픽_covid19.xlsx")
...
tidy(lda_data,matrix = "gamma") %>% 
  ...
  summarise(mean_p = mean(gamma)) %>% 
  arrange(topic) %>% 
  openxlsx::write.xlsx("../result/oc/covid19/붙임5.토픽출현율_평균_covid19.xlsx")
...
data_oc_post = data_oc %>% mutate(
  article = str_replace_all(keyword,","," ")
) %>% filter(covid_type=="post_covid19")


cps <- VCorpus(VectorSource(data_oc_post$article))
cps = tm_map(cps, removePunctuation)
cps = tm_map(cps, removeNumbers)
...
cps = tm_map(cps, myRemove, '현대로템')
cps = tm_map(cps, myRemove, '위원장')
cps = tm_map(cps, myRemove, '청소년')
cps = tm_map(cps, myRemove, '공직자')
...
tidy(lda_data,matrix = "gamma") %>% 
  ...
  openxlsx::write.xlsx("../result/oc/post_covid/붙임2.토픽별대표문서_post_covid19.xlsx")

tidy(lda_data,matrix = "gamma") %>% 
  group_by(topic) %>% 
  slice(which.max(gamma)) %>% 
  openxlsx::write.xlsx("../result/oc/post_covid/붙임3.토픽별대표문서리스트_post_covid19.xlsx")
...
tidy(lda_data,matrix = "gamma") %>% 
  ...
  slice(which.max(gamma)) %>% 
  arrange(as.numeric(document)) %>% 
  openxlsx::write.xlsx("../result/oc/post_covid/붙임4.문서별토픽_post_covid19.xlsx")
...
tidy(lda_data,matrix = "gamma") %>% 
  ...
  summarise(mean_p = mean(gamma)) %>% 
  arrange(topic) %>% 
  openxlsx::write.xlsx("../result/oc/post_covid/붙임5.토픽출현율_평균_post_covid19.xlsx")
...
lda_term_top[lda_term_top$term=="마케팅",2]<-"소통감소"
ggplot(lda_term_top, aes(reorder(term, beta), beta, fill=factor(topic))) +
  geom_col(show.legend = FALSE) +
  facet_wrap(~topic,scales="free")+
  labs(x=NULL, y="Word-Topic Probability (Beta)") + coord_flip()+
